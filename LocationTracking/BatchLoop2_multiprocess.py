...
def do_one_file(pool_count, total_count, filename):

    # Remove .avi suffix
    filepath_without_ext = Path(filename).with_suffix("")

    if "tracked" in filename.name:
        print(f'\nskipping tracked file {pool_count + 1}: {filename}')
        return

    print(f'\n**** FILE {pool_count + 1} OF {total_count}: {filename}')

    video_dict = {
        'dpath'        : filename.parent,   # Path to parent folder
        'file'         : filename.name,     # File name only, without directories
    ...
    animal_id_suffix = filename.name[16:]
...
if __name__ == '__main__':
    ...
    print(
        '\nPlease select behavioral videos folder from dialog box. Note that dialog might be behind the Python window, or on another screen.')
